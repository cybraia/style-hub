...
@app.route('/virtual-tryon/generate', methods=['POST'])
def generate_virtual_tryon():
    """
    Generates a virtual try-on image using Google Gemini API.
    Expects two files: 'person_image' and 'clothing_image'
    """
    try:
        ...
        person_image_data = person_file.read()
        clothing_image_data = clothing_file.read()
...
"""
        ...
        response = client.models.generate_content(
            ...
            contents=[
                prompt,
                {
                    'mime_type': person_file.content_type or 'image/jpeg',
                    'data': person_image_data,
                },
                {
                    'mime_type': clothing_file.content_type or 'image/jpeg',
        ...
        return jsonify({"error": "No image was generated by the model."}), 500
        
    except Exception as e:
        print(f"Error during virtual try-on generation: {e}")
        return jsonify({"error": "Failed to generate virtual try-on.", "details": str(e)}), 500
